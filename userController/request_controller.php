...
if (isset($_POST['request'])) {
    ...
    if ($_POST['requestor'] === "me") {
        ...
        $insert_original = [
            'query' => "INSERT INTO document_request (user_id, doc_type_id, request_name, request_purpose, request_address)
                        VALUES (?,?,?,?,?)",
            'bind' => 'iisss',
            'value' => [$_SESSION['user_id'], $select_document, $originalUser, $select_purpose, $_SESSION['user_purok']]
        ];

        $result = insertData($insert_original);

        if ($result) {
            echo "<script>alert('1'); window.location.href='../request.php';</script>";
        } else {
        ...
        $address = ($_POST['addressOption'] === "sameAsMe") ? $_SESSION['user_purok'] : $_POST['otherAddress'] ;
