window.addEventListener('scroll', function() {
    ...
    const chatCount = document.querySelector(
      "header nav .notifications span.notif-count"
    ); // Notification count
    if (chatCount) chatCount.style.color = "#fff";
    ...
    if (window.scrollY > 0) {
        ...
        if (notificationCount) notificationCount.style.color = '#fff';
         // Change notification count color back to white
    }
});
